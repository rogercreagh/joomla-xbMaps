...
class XbmapsTableMarker extends JTable
{
	...
	public function check() {
		...
		if (($this->id == 0) && (XbmapsHelper::checkTitleExists($title,'#__xbmaps_markers'))) {
		...
		if (!$this->catid>0) {
			...
			if ($defcat>0) {
				$this->catid = $defcat;
				if (($params->get('global_use_cats') && ($params->get('markers_use_cats')))) {
