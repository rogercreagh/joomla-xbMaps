...
class XbmapsModelTracklist extends JModelList {
	...
	protected function getListQuery() {
		...
		if ($params->get('tracks_with_maps')) {
	       $query->join('right','#__xbmaps_maptracks AS mt ON mt.track_id=a.id');
	    }
		
		$query->select('c.title AS category_title');
		$query->join('LEFT', '#__categories AS c ON c.id = a.catid');
		//if only tracks_with_maps need to filter out unassigned tracks
...
// 		}
		...
		if (($taglogic === '2') && (empty($tagfilt))) {
			...
			$subQuery = '(SELECT content_item_id FROM #__contentitem_tag_map
 				WHERE type_alias = '.$db->quote('com_xbmaps.track').')';
				...
				if (count($tagfilt)==1)	{ //simple version for only one tag
					$query->join( 'INNER', $db->quoteName('#__contentitem_tag_map', 'tagmap')
							...
							->where(array( $db->quoteName('tagmap.tag_id') . ' = ' . $tagfilt[0],
									$db->quoteName('tagmap.type_alias') . ' = ' . $db->quote('com_xbmaps.track') )
									);
					...
					if ($taglogic == 1) { // match ALL listed tags
						...
						for ($i = 0; $i < count($tagfilt); $i++) {
							...
							$query->where( array(
									...
									$db->quoteName($mapname.'.type_alias') . ' = ' . $db->quote('com_xbmaps.track'))
									);
						}
						...
						->where( array(
								...
								$db->quoteName('type_alias') . ' = ' . $db->quote('com_xbmaps.track'))
								);
						$query->join('INNER',
	...
	public function getItems() {
		...
		$app->setUserState('tracks.sortorder', $mps);
		
		foreach ($items as $i=>$item) {
			$item->tags = $tagsHelper->getItemTags('com_xbmaps.track' , $item->id);
    		$item->maps = XbmapsGeneral::trackMapsArray($item->id,1);
    		$item->markers = XbmapsGeneral::trackMarkersArray($item->id,1);;
