...
class XbmapsModelMarkerlist extends JModelList {
    ...
    protected function getListQuery() {
        ...
        //only get markers assigned to maps
        $query->join('RIGHT', '#__xbmaps_mapmarkers AS mm ON mm.marker_id = a.id');
        ...
        if (is_numeric($categoryId)) {
            ...
            if (($taglogic === '2') && (empty($tagfilt))) {
                ...
                $subQuery = '(SELECT content_item_id FROM #__contentitem_tag_map
 				WHERE type_alias = '.$db->quote('com_xbmaps.marker').')';
                    ...
                    if (count($tagfilt)==1)	{ //simple version for only one tag
                        $query->join( 'INNER', $db->quoteName('#__contentitem_tag_map', 'tagmap')
                            ...
                            ->where(array( $db->quoteName('tagmap.tag_id') . ' = ' . $tagfilt[0],
                                $db->quoteName('tagmap.type_alias') . ' = ' . $db->quote('com_xbmaps.marker') )
                                );
                        ...
                        if ($taglogic == 1) { // match ALL listed tags
                            ...
                            for ($i = 0; $i < count($tagfilt); $i++) {
                                ...
                                $query->where( array(
                                    ...
                                    $db->quoteName($mapname.'.type_alias') . ' = ' . $db->quote('com_xbmaps.marker'))
                                    );
                            }
                            ...
                            ->where( array(
                                ...
                                $db->quoteName('type_alias') . ' = ' . $db->quote('com_xbmaps.marker'))
                                );
                            $query->join('INNER',
